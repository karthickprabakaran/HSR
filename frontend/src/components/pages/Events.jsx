...
import headerImg from "../../assets/images/longentrance_1.jpg";
...
const Events = () => (
  <>
    <header
      className="w-full h-[60vh] bg-center bg-cover bg-no-repeat flex items-center justify-center relative"
      style={{ backgroundImage: `url(${headerImg})` }}
    >
      <div className="bg-black/50 px-6 py-8 rounded-md text-center max-w-2xl">
        <h1 className="text-4xl font-bold text-white mb-3">
          Welcome to The Celebration Lawn
        </h1>
        <p className="text-lg text-white">
          Our beautifully curated open-air lawn is the perfect setting for
          weddings, private gatherings, corporate events, and unforgettable
          celebrations.
        </p>
      </div>
    </header>
    <div className="pt-24 min-h-screen bg-gray-50 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-lg text-gray-600 font-medium mb-8 text-center">
          Ideal for 250-300 Seatings. The perfect venue for weddings, parties,
        ...
        <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {eventTypes.map((event, idx) => (
            <div
              key={idx}
              className="relative rounded-xl shadow-lg bg-white overflow-hidden flex items-end group min-h-[270px]"
            >
              <img
                src={event.img}
                alt={event.title}
              ...
              <div className="relative z-10 w-full p-6 text-center">
                <h2 className="text-xl font-semibold text-white drop-shadow-lg tracking-wide uppercase">
                  {event.title}
                </h2>
              </div>
            </div>
