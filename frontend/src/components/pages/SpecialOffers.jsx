...
const SpecialOffers = () => {
  return (
    <div className="pt-24 min-h-screen bg-gray-50 flex flex-col items-center px-4">
      <h1 className="text-3xl font-bold text-blue-700 mb-10 text-center">Special Offers</h1>
      <div className="w-full max-w-5xl space-y-12">
        {offers.map((offer, idx) => (
          <div
            key={idx}
            className={`flex flex-col md:flex-row ${idx % 2 === 1 ? 'md:flex-row-reverse' : ''} items-center bg-white rounded-2xl shadow-lg overflow-hidden`}
          >
            {/* Image */}
            <div className="md:w-1/2 w-full h-64 md:h-[280px] flex-shrink-0 flex items-center justify-center bg-gray-100">
              <img
                src={offer.img}
                alt={offer.title}
            ...
            <div className="md:w-1/2 w-full h-full p-8 flex flex-col justify-center text-center md:text-left">
              <h2 className="text-2xl font-semibold text-blue-700 mb-2">{offer.title}</h2>
              <p className="text-gray-700 text-base mb-1">{offer.description}</p>
            </div>
          </div>
        ))}
      </div>
